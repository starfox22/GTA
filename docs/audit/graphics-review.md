...
| Helicopter searchlight made the player less visible on clear nights (a white layer over them) | The shaft was marched down to the ground plane through the player (it cannot read the depth buffer it draws into) and, seen end-on by the street camera, saturated at 2.2 HDR; the pool was ~4x too bright, clipping pale paving (18-22% of the pool at 255) and blooming over the player. Player vs pool Weber contrast -0.03 (-0.07 without the shaft) | Pool exposed against `postLook.exposure` (no clipping; contrast -0.68..-0.73), crisp xenon cookie, shaft faint in clear air, gone over its last stretch and round the target, HIGH/ULTRA only (searchlight3d.js `AIR_LIGHT`) |
