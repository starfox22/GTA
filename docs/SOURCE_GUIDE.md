...
| hud.js | HUD behaviour: pop-open radio and weapon boxes (`hudPop`), minimap fold and zoom (`hudState`), the SPEED BOX (`updateSpeedBox`, `trackPlayerPace`, the km/h / mph units: `speedReading`, `speedText`, `kmhReading`), wanted stars, context key hints, the HOW TO PLAY key grid; the title menu (`updateTitleMenu`); PANEL COVER (`syncPanelCover`, `hudCovered`: body.panel-open hides the toast, headline card, district plate, navigation pill, story line and pager under any full-screen panel; a toast raised while one is open is tagged `.over-panel` and stays) |
