...
| `helicopterLineup(x, y, heading, rotors, spacing)`, `helicopterModels()` | Park one helicopter of each look (police, news, executive, military) in a row, rotors spun up and police lights running with `rotors` true (helicopter3d.js); every helicopter model built: look, rotor spool, draw calls, shadow casters, triangles, crew shown |
