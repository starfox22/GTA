...
- `window.DeadEndCity` (game-console.js + game-console-world.js + game-console-graphics.js:
  one frozen object literal split over three files) is the only way tests reach the game.
- It has **explicit named methods only**. Never add a generic code-evaluation hook (eval,
  ...
  needs something, add a named method that does exactly that and document it in the table.
...
Grep this table rather than reading it whole: `grep -n 'brakeTest' docs/areas/testing-and-console.md`.
