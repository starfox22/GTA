      ...
      const roadblockPool = Array.from({ length: ROADBLOCK_POOL }, () => {
        ...
        // Spike strip: a dark mat with a saw edge, laid across the gap.
        const strip = new Three.Group();
        g.add(strip);
        box(strip, 0, 0.9, 0, 9, 1.8, 104, spikeMat);
        for (let i = -6; i <= 6; i++)
          mesh(cylinderGeo, spikeMat, strip, 0, 3.4, i * 8, 0.8, 4, 0.8);
        const cones = [];
        for (let i = -2; i <= 2; i++) {
          if (!i) continue;
          const c = new Three.Group();
          c.position.set(i * 9, 0, i * 22);
          g.add(c);
        ...
        const flares = [-1, 1].map((side) => {
          const f = mesh(sphereGeo, flareMat, g, 0, 2.5, side * 44, 2.4, 2.4, 2.4);
          f.castShadow = false;
